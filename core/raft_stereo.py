    ...
    def forward(self, image_pair, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            *cnet_list, fmap1, fmap2 = self.cnet(image_pair)
            ...
            inp_list = [list(conv(i).split(split_size=conv.out_channels // 3, dim=1)) for i, conv in zip(inp_list, self.context_zqr_convs)] # 2 96 128 128

        # run update module
        flow_pred = self.update_module(fmap1, fmap2, net_list, inp_list, iters, flow_init, test_mode)
