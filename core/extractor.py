...
class ResidualBlockDilated(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1, dilation=2):
...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1):
        ...
        # 添加注意力模块
        # self.attention = SEWithAdvancedSpatialAttention(planes)# 添加注意力模块
...
class UnetExtractor(nn.Module):
    def __init__(self, in_channel=3, encoder_dim=[64, 96, 128], norm_fn='group'):
        ...
        self.res1 = nn.Sequential(
            ResidualBlockDilated(32, encoder_dim[0], norm_fn=norm_fn),
            ResidualBlockDilated(encoder_dim[0], encoder_dim[0], norm_fn=norm_fn)
        )
        self.res2 = nn.Sequential(
            ResidualBlockDilated(encoder_dim[0], encoder_dim[1], stride=2, norm_fn=norm_fn),
            ResidualBlockDilated(encoder_dim[1], encoder_dim[1], norm_fn=norm_fn)
        )
        self.res3 = nn.Sequential(
            ResidualBlockDilated(encoder_dim[1], encoder_dim[2], stride=2, norm_fn=norm_fn),
            ResidualBlockDilated(encoder_dim[2], encoder_dim[2], norm_fn=norm_fn),
        )
...
class MultiBasicEncoder(nn.Module):
    def __init__(self, output_dim=[128], encoder_dim=[64, 96, 128]):
        ...
        self.conv2 = nn.Sequential(
            ResidualBlockDilated(encoder_dim[2], encoder_dim[2], stride=1),
