...
def disp2depth(data):
    data['lmain']['disp_const'] = data['lmain']['disp_const'].view(data['lmain']['img'].shape[0], 1, 1, 1)
    depth = data['lmain']['disp_const'] / data['lmain']['flow_pred']  # f*b/d
    depth = (depth * data['lmain']['mask']).float()
    # depth = (depth).float()
    depth = (depth - depth.min().item()) / (depth.max().item() - depth.min().item())
    return depth



def PseudoColor(disp_pred_lmain):
    # grad解放
    disp_pred_lmain = disp_pred_lmain.detach()  # 2 1 1024 1024

    # 将图像从 CUDA 转移到 CPU 并转换为 numpy 数组   去掉单通道维度
    image_numpy = disp_pred_lmain.squeeze().cpu().numpy() # 6 1024 1024

    # 使用 matplotlib 的颜色映射（例如 'viridis'）将图像转换为伪彩色图像
    colormap = cm.get_cmap('viridis')

    image_rgb = colormap(image_numpy)[:, :, :3]  # 取前三个通道（RGB）
    image_rgb_tensor = torch.from_numpy(image_rgb).permute(2, 0, 1)  # 转换为 (C, H, W) 格式
