...
class Trainer:
    ...
    def train(self):
        for _ in tqdm(range(self.total_steps, self.cfg.num_steps)):
            ...
            #  Gaussian Render
            data, _ = pts2render(data, bg_color=self.cfg.dataset.bg_color)

            render_novel = data['lmain']['img_pred'] #0 1
            gt_novel = (data['lmain']['img'].cuda()+1.0)/2 #-1 1
            Ll1 = l1_loss(render_novel, gt_novel)
            Lssim = 1.0 - ssim(render_novel, gt_novel)
            loss = 1.0 * disp_loss + 0.8 * Ll1 + 0.2 * Lssim
            ...
            if self.total_steps and self.total_steps % self.cfg.record.loss_freq == 0:
                ...
                self.logger.writer.add_image('gt_l',(data['lmain']['img'][0]+1.0)/2, self.total_steps)
                self.logger.writer.add_image('pred_depth',PseudoColor(data['lmain']['depth_pred'][0]), self.total_steps)
                self.logger.writer.add_image('gt_depth',PseudoColor(data['lmain']['depth'][0]), self.total_steps)
                self.logger.writer.add_image('new_l',data['lmain']['img_pred'][0], self.total_steps)
    ...
    def run_eval(self):
        ...
        for idx in range(self.len_val):
            ...
            with torch.no_grad():
                ...
                render_novel = data['lmain']['img_pred']
                gt_novel = (data['lmain']['img'].cuda()+1.0)/2

                mask = data['lmain']['mask'].expand(-1, 3, -1, -1).bool()
                render_novel = render_novel * mask
                gt_novel = gt_novel * mask
                psnr_value = psnr(render_novel, gt_novel).mean().double()
                psnr_list.append(psnr_value.item())

                # mask2 = data['lmain']['img_pred'] * (data['lmain']['img_pred']!=0)
                # render_novel2 = render_novel * mask2
                # gt_novel2 = gt_novel * mask2
                # psnr_value2 = psnr(render_novel2, gt_novel2).mean().double()
                # psnr_list.append(psnr_value2.item())
